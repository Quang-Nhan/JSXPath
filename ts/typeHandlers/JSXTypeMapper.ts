...
export class JSXTypeMapper {
  ...
  private getType(chars: string, handlerId:number = 0, prevInstruction?: Instruction): string {
    ...
    if (nextTypeChecks.length) {
      const type = nextTypeChecks.find(this.isMatchedType(chars, handlerId))
      // if (!type) {
      //   throw new Error('Was expecting ' + chars + ' to be one of ' + nextTypeChecks.join(', '));
      // }
      return type;
    }
    return this.ALL_TYPES.find(this.isMatchedType(chars, handlerId));
  }

  private isMatchedType(chars: string, handlerId:number) {
    return (type: string) => this.PathScenarios.test(chars, handlerId)(type);
  }

  getTypeHandler(props: {subPath?:string, prevInstruction?:Instruction, type?:string, handlerId?:number}) {
    let {subPath, prevInstruction, type, handlerId} = props;
    if (!type) {
      type = this.getType(subPath, handlerId, prevInstruction);
    }
