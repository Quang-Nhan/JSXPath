...
export class Postfix {
  private cache: Map<string, Array<tStack | tStack[]>> = new Map();
  private props = {
    trimmedInput: null,
    pathIndex: null
  };
  ...
  public toPostfix(inputPath: string) {
    ...
    if (this.cache.has(trimmedPath)) {
      // Return a deep copy to avoid mutation issues
      return JSON.parse(JSON.stringify(this.cache.get(trimmedPath)));
    }

    this.props.trimmedInput = trimmedPath;
    let subPath = '';
    ...
    this.cache.set(trimmedPath, result);
